...
The next remediation must first prove these workflow changes with faux-provider/unit fixtures and Controller catalog tests. It then reruns only the three remaining logical tasks in a new labelled cohort. Success criteria are: no P2P regression for `4316`; 1/1 F2P and preserved P2P for each task; a retained terminating V0/V1/V2 record; and a PLAN-to-diff coverage record that accounts for every claimed semantic obligation. No Pi rerun and no R2 ablation starts as part of this follow-up.
